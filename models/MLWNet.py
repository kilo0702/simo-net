...
class Fusion(nn.Module):
    ...
    def get_wavelet_loss(self):
        wavelet_loss = 0.
        for index, val in enumerate(self.num_blocks):
            if val is not None:
...
def build_net(model_name):
    if model_name == "MLWNet-B":
        # B 版：基礎通道數 64
        return MLWNet_Local(dim=64, train_size=(1, 3, 256, 256))
    elif model_name == "MLWNet-S":
        # S 版：基礎通道數 32，運算量小很多
        return MLWNet_Local(dim=32, train_size=(1, 3, 256, 256))
    else:
        raise ValueError(f'Wrong Model Name: {model_name}! You should choose MLWNet-B or MLWNet-S.')
